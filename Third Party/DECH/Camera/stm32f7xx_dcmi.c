...
void DCMI_Init(DCMI_InitTypeDef* DCMI_InitStruct)
{
  ...
  temp &= ~(DCMI_CR_PCKPOL | DCMI_CR_HSPOL  | DCMI_CR_VSPOL  | DCMI_CR_EDM_0 |\
            DCMI_CR_EDM_1  | DCMI_CR_FCRC_0 | DCMI_CR_FCRC_1 | DCMI_CR_JPEG  |\
            DCMI_CR_ESS | DCMI_CR_BSM_0 | DCMI_CR_BSM_1 | DCMI_CR_OEBS |\
            DCMI_CR_LSM | DCMI_CR_OELS);
  
  /* Sets the new configuration of the DCMI peripheral */
  temp |= ((uint32_t)DCMI_InitStruct->CaptureMode |
                     DCMI_InitStruct->SynchroMode |
                     DCMI_InitStruct->PCKPolarity |
                     DCMI_InitStruct->VSPolarity |
                     DCMI_InitStruct->HSPolarity |
                     DCMI_InitStruct->CaptureRate |
                     DCMI_InitStruct->ExtendedDataMode |
                     DCMI_InitStruct->JPEGMode | 
                     DCMI_InitStruct->ByteSelectMode |
                     DCMI_InitStruct->ByteSelectStart | 
                     DCMI_InitStruct->LineSelectMode |
...
void DCMI_StructInit(DCMI_InitTypeDef* DCMI_InitStruct)
{
  ...
  DCMI_InitStruct->CaptureMode = DCMI_MODE_CONTINUOUS;
  DCMI_InitStruct->SynchroMode = DCMI_SYNCHRO_HARDWARE;
  DCMI_InitStruct->PCKPolarity = DCMI_PCKPOLARITY_FALLING;
  ...
  DCMI_InitStruct->JPEGMode = DCMI_JPEG_DISABLE;
  ...
  DCMI_InitStruct->LineSelectStart = DCMI_OELS_ODD;
}
...
{  
  ...
  DCMI->CWSTRTR = (uint32_t)((uint32_t)DCMI_CROPInitStruct->HorizontalOffsetCount |
                  ((uint32_t)DCMI_CROPInitStruct->VerticalStartLine << 16));

  /* Sets the CROP window size */
  DCMI->CWSIZER = (uint32_t)(DCMI_CROPInitStruct->CaptureCount |
                  ((uint32_t)DCMI_CROPInitStruct->VerticalLineCount << 16));
}
...
/**
  * @brief  Initializes the DCMI peripheral Capture mode according to the specified
  *         parameters in the DCMI_CaptureMode.
  * @note   This function should be called before to enable and start the DCMI interface.   
  * @param  DCMI_CaptureMode:   DCMI_MODE_CONTINUOUS
  *                             DCMI_MODE_SNAPSHOT
  * @retval None
  */
...
{  
  ...
  DCMI->CR &= ~(DCMI_CR_CM);
  DCMI->CR |=  (uint32_t)(DCMI_CaptureMode);
}
